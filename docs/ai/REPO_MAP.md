...
- Request lifecycle transitions are now written to `audit` as compact operator-facing events: `request_submitted`, `request_running`, `request_complete`, and `request_failed`.
- Failed status transitions continue to include structured metadata (`reason`, `stage`, `category`) using bounded enums (`FailureStage`, `FailureCategory`) and are read back by API/UI (`failure_reason`, `failure_stage`, `failure_category`).
...
- UI request page now also renders a compact `Request lifecycle` section from persisted audit events, with fallback to request/result timestamps when older records predate the new audit actions.
