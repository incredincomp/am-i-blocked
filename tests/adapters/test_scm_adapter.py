...
class TestSCMAdapterEvidence:
    ...
    @pytest.mark.anyio
    @respx.mock
    async def test_query_evidence_missing_source_marker_deny_like_candidate_returns_empty(self):
        adapter = SCMAdapter(
            client_id="cid",
            client_secret="sec",
            tsg_id="tsg",
        ...
        respx.post("https://api.example.com/scm/query").mock(
            return_value=httpx.Response(
                ...
                json={
                    "records": [
                        {
                            # Intentionally missing source-of-record marker (source_system/system_of_record).
                            "authoritative": True,
                            "decision": "deny",
                            "destination": "api.example.com",
                            "port": 443,
                            "timestamp": "2026-03-11T00:00:00Z",
                            "rule_name": "cloud-block",
                            "reason": "Policy deny candidate missing source marker",
                        }
                    ]
                },
            )
    ...
    @pytest.mark.anyio
    @respx.mock
    async def test_query_evidence_invalid_source_marker_deny_like_candidate_returns_empty(self):
        adapter = SCMAdapter(
            client_id="cid",
            client_secret="sec",
            tsg_id="tsg",
        ...
        respx.post("https://api.example.com/scm/query").mock(
            return_value=httpx.Response(
                ...
                json={
                    "records": [
                        {
                            # Intentionally non-SCM source marker; deny-like shape should still fail closed.
                            "source_system": "third_party_unknown",
                            "authoritative": True,
                            "decision": "deny",
                            "destination": "api.example.com",
                            "port": 443,
                            "timestamp": "2026-03-11T00:00:00Z",
                            "rule_name": "cloud-block",
                            "reason": "Policy deny candidate with invalid source marker",
                        }
                    ]
                },
            )
    ...
    @pytest.mark.anyio
    @respx.mock
    async def test_query_evidence_blank_destination_deny_like_candidate_returns_empty(self):
        adapter = SCMAdapter(
            client_id="cid",
            client_secret="sec",
            tsg_id="tsg",
        ...
        respx.post("https://api.example.com/scm/query").mock(
            return_value=httpx.Response(
                ...
                json={
                    "records": [
                        {
                            # Intentionally blank destination while otherwise deny-like and authoritative.
                            "source_system": "strata_cloud_manager",
                            "authoritative": True,
                            "decision": "deny",
                            "destination": "   ",
                            ...
                            "timestamp": "2026-03-11T00:00:00Z",
                            "rule_name": "cloud-block",
                            "reason": "Policy deny candidate with blank destination",
                        }
                    ]
                },
            )
    ...
    @pytest.mark.anyio
    @respx.mock
    async def test_query_evidence_object_destination_deny_like_candidate_returns_empty(self):
        adapter = SCMAdapter(
            client_id="cid",
            client_secret="sec",
            tsg_id="tsg",
        ...
        respx.post("https://api.example.com/scm/query").mock(
            return_value=httpx.Response(
                ...
                json={
                    "records": [
                        {
                            # Intentionally object-typed destination while otherwise deny-like and authoritative.
                            "source_system": "strata_cloud_manager",
                            "authoritative": True,
                            "decision": "deny",
                            ...
                            "timestamp": "2026-03-11T00:00:00Z",
                            "rule_name": "cloud-block",
                            "reason": "Policy deny candidate with malformed destination type",
                        }
                    ]
                },
            )
    ...
    @pytest.mark.anyio
    @respx.mock
    async def test_query_evidence_object_timestamp_deny_like_candidate_returns_empty(self):
        adapter = SCMAdapter(
            client_id="cid",
            client_secret="sec",
            tsg_id="tsg",
        ...
        respx.post("https://api.example.com/scm/query").mock(
            return_value=httpx.Response(
                ...
                json={
                    "records": [
                        {
                            # Intentionally object-typed timestamp while otherwise deny-like and authoritative.
                            "source_system": "strata_cloud_manager",
                            "authoritative": True,
                            "decision": "deny",
                            "destination": "api.example.com",
                            "port": 443,
                            "timestamp": {"raw": "2026-03-11T00:00:00Z"},
                            "rule_name": "cloud-block",
                            "reason": "Policy deny candidate with malformed timestamp type",
                        }
                    ]
                },
            )
